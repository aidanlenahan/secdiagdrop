...
function Get-ProblemsFixed {
    Open-Sec 'Problems fixed'
    $fixed=@()
    if($NonInteractive){ $fixed=@('(not recorded)') }
    else{
        Line 'What did you fix? One per line, blank to finish:' 'Dim'
        while($true){Write-Host ($G.V+'    - ') -NoNewline -ForegroundColor $UI.Accent;$p=Read-Host;if([string]::IsNullOrWhiteSpace($p)){break};$fixed+=$p.Trim()}
    }
    if(-not $fixed){$fixed=@('(none reported)')}
    Close-Sec
    $fixed
}
#endregion
...
# Domains that are legit infrastructure - never score these as risky.
$AllowDomains=@('*.microsoft.com','*.microsoftonline.com','*.microsoft','*.office.com','*.office365.com','*.live.com','*.windows.net','*.azure.com','*.azureedge.net','*.cloud.microsoft','*.onecdn.static.microsoft','*.google.com','*.gstatic.com','*.googleapis.com','*.googleusercontent.com','*.gmail.com','*.apple.com','*.icloud.com','*.amazonaws.com','*.cloudfront.net','*.akamaized.net','*.akamai.net','*.fastly.net','*.cloudflare.com','*.github.com','*.githubusercontent.com','*.fidelity.com','*.chase.com','*.bankofamerica.com','*.wellsfargo.com','*.paypal.com','*.mozilla.org','*.mozilla.net')
function Test-PrivateIP{param($d)
    # RFC1918, loopback, link-local, and CGNAT/Tailscale 100.64-100.127
    if($d -notmatch '^(\d{1,3}\.){3}\d{1,3}$'){return $false}
    $o=$d.Split('.')|ForEach-Object{[int]$_}
    ($o[0]-eq10) -or ($o[0]-eq127) -or ($o[0]-eq0) -or ($o[0]-eq169 -and $o[1]-eq254) -or ($o[0]-eq192 -and $o[1]-eq168) -or ($o[0]-eq172 -and $o[1]-ge16 -and $o[1]-le31) -or ($o[0]-eq100 -and $o[1]-ge64 -and $o[1]-le127)}
function Get-DomainRisk{param($Domain,$Url='')$s=0;$r=@();if(-not $Domain){return $null};$d=$Domain.ToLower()
    if(Test-PrivateIP $d){return $null}                    # internal/VPN IPs are not suspicious
    foreach($a in $AllowDomains){if($d -like $a){return $null}}  # trusted infrastructure
    foreach($t in $SuspiciousTlds){if($d.EndsWith($t)){$s+=25;$r+="abuse TLD $t";break}}
    if($d -match '^(\d{1,3}\.){3}\d{1,3}$'){$s+=30;$r+='raw public IP host'}
    ...
    foreach($b in 'paypal','microsoft','apple','amazon','chase','coinbase','geeksquad','norton'){if($d -match $b -and $d -notmatch "(^|\.)$b\.(com|net|org|co\.uk)$"){$s+=35;$r+="brand '$b' off-domain";break}}
    if($h.Length -ge 12){$grp=$h.ToCharArray()|Group-Object;$e=0.0;foreach($g in $grp){$p=$g.Count/$h.Length;$e+=-1*$p*[math]::Log($p,2)};if($e -gt 3.9){$s+=20;$r+=('entropy {0:N2}'-f$e)}}
...
function Get-DeviceIdentity{
    ...
    foreach($prov in @(
        @{U='https://ipapi.co/json/';       IP={param($x)$x.ip};    G={param($x)"$($x.city), $($x.region), $($x.country_name) (ISP $($x.org))"}}
        @{U='http://ip-api.com/json/';       IP={param($x)$x.query}; G={param($x)"$($x.city), $($x.regionName), $($x.country) (ISP $($x.isp))"}}
        @{U='https://ipwho.is/';             IP={param($x)$x.ip};    G={param($x)"$($x.city), $($x.region), $($x.country) (ISP $($x.connection.isp))"}}
        @{U='https://ipinfo.io/json';        IP={param($x)$x.ip};    G={param($x)"$($x.city), $($x.region), $($x.country) (ISP $($x.org))"}}
        @{U='https://api.ipify.org?format=json'; IP={param($x)$x.ip}; G={param($x)$null}}
    )){
        try{ $resp=Invoke-RestMethod $prov.U -TimeoutSec 8 -EA Stop
             $maybeIp=& $prov.IP $resp
             if($maybeIp){ if(-not $pub){$pub=$maybeIp}; $g=& $prov.G $resp; if($g -and $g -notmatch '^\s*,'){$geo=$g}; if($pub -and $geo){break} } }
        catch{}
    }
...
# ============================================================================
function New-Diagnostic{param($Intake,$System,$Disks,$Smart,$Files,$Apps,$Flagged,$Startup,$Browser,$Updates,$Device,$ProblemsFixed,$ActionsLog)
    ...
    if($ProblemsFixed){[void]$sb.AppendLine('## Problems fixed').AppendLine();foreach($p in $ProblemsFixed){[void]$sb.AppendLine("- $p")};[void]$sb.AppendLine()}
    if($ActionsLog){[void]$sb.AppendLine('## Actions taken').AppendLine();foreach($a in $ActionsLog){[void]$sb.AppendLine("- $a")};[void]$sb.AppendLine()}
...
function Invoke-ActionQueue{
    ...
    $log=@()
    if(-not $script:Actions.Count){Stat 'No actions proposed.' 'Ok';Close-Sec;return $log}
    if($NonInteractive){Stat "$($script:Actions.Count) actions proposed but skipped (non-interactive)." 'Info';Close-Sec;return $log}
    ...
    foreach($a in ($script:Actions|Sort-Object @{e={switch($_.Category){'Safeguarding'{0}'Credentials'{1}'Updates'{2}default{3}}}})){
        ...
        if(Ask 'Do it?'){try{& $a.Do $a.Ctx;$done++;$log+="[$($a.Category)] $($a.Summary) - DONE"}catch{Stat "Action failed: $($_.Exception.Message)" 'Warn';$log+="[$($a.Category)] $($a.Summary) - FAILED: $($_.Exception.Message)"}}else{Stat 'Skipped.' 'Info';$log+="[$($a.Category)] $($a.Summary) - skipped"};Div}
...
try {
    ...
    # Do all the fixing FIRST, so the diagnostic can record it.
    $actionsLog = Invoke-ActionQueue
    $fixed      = Get-ProblemsFixed
    ...
    $md = New-Diagnostic -Intake $intake -System $sys -Disks $disks -Smart $smart -Files $files -Apps $apps -Flagged $flagged -Startup $startup -Browser $browser -Updates $updates -Device $device -ProblemsFixed $fixed -ActionsLog $actionsLog
    ...
    if($url){KV 'Uploaded' $url 'Ok'}
        ...
        # Upload failed or was denied. Offer a local save to the Desktop.
        Stat 'Upload did not complete.' 'Warn'
        $desktop = [Environment]::GetFolderPath('Desktop')
        $fallbackFile = if($LocalFallbackPath){$LocalFallbackPath}
                        else{Join-Path $desktop ("DIAG-{0}-{1}.md" -f ($intake.ComputerNumber -replace '[^\w\-]','_'),(Get-Date -Format 'yyyy-MM-dd_HHmmss'))}
        if($NonInteractive){
            try{$md|Out-File $fallbackFile -Encoding UTF8;Stat "Saved locally: $fallbackFile" 'Ok'}catch{Stat "Local save failed: $($_.Exception.Message)" 'Warn'}
        } elseif(Ask "Save the diagnostic to the Desktop instead? ($fallbackFile)" -DefaultYes){
            try{$md|Out-File $fallbackFile -Encoding UTF8;Stat "Saved: $fallbackFile" 'Ok'
                if($script:Safeguard.Count){Stat 'NOTE: this file contains safeguarding items and is on the local Desktop. Move it to your overseers and delete the local copy ASAP.' 'Warn'}}
            catch{
                Stat "Desktop save failed: $($_.Exception.Message)" 'Warn'
                if(Ask 'Try a different folder?'){
                    $alt=Read-Host '   Full path (including filename.md)'
                    try{$md|Out-File $alt -Encoding UTF8;Stat "Saved: $alt" 'Ok'}catch{Stat "Save failed: $($_.Exception.Message)" 'Warn'}
                }
            }
        } else {
            Stat 'Not saved. Nothing left on this machine.' 'Info'
        }
    }
    Close-Sec
